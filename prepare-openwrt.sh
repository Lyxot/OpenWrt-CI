...
if [[ ! -f "$ci_dir/.config" || ! -f "$ci_dir/busybox.py" ]]; then
...
# AUTO_LATEST_TOOLCHAIN=0 to preserve the version selected in CI/.config.
...
make defconfig
diff -u .config.raw .config | tee .config.diff || true
